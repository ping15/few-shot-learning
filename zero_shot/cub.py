...
def main():
    ...
    if os.path.exists("./models/zsl_cub_attribute_network_v35.pkl"):
    ...
    if os.path.exists("./models/zsl_cub_relation_network_v35.pkl"):
    ...
    for episode in range(EPISODE):
        ...
        batch_features, batch_labels = next(train_loader.__iter__())
        ...
        sample_attributes = torch.Tensor(np.array([all_attributes[i] for i in sample_labels])).squeeze(1)
        # sample_attributes = torch.Tensor([all_attributes[i] for i in sample_labels]).squeeze(1)
        class_num = sample_attributes.shape[0]

        batch_features = torch.tensor(batch_features).clone().detach().float()
        # batch_features = torch.tensor(batch_features).float()  # 32*1024
        sample_features = attribute_network(torch.tensor(sample_attributes).clone().detach())
        # sample_features = attribute_network(torch.tensor(sample_attributes)) # k*312
        ...
        # loss
        mse = nn.MSELoss()
        # one_hot_labels = torch.tensor(torch.zeros(BATCH_SIZE, class_num).scatter_(1, re_batch_labels.view(-1, 1), 1))
        one_hot_labels = torch.zeros(BATCH_SIZE, class_num).scatter_(1, re_batch_labels.view(-1, 1), 1).clone().detach()
        ...
        if (episode + 1) % 200 == 0:
            ...
            def compute_accuracy(test_features, test_label, test_id, test_attributes):
                ...
                for batch_features, batch_labels in test_loader:
                    ...
                    batch_features = torch.tensor(batch_features).clone().detach().float()  # 32*1024
                    sample_features = attribute_network(torch.tensor(sample_attributes).clone().detach().float())
